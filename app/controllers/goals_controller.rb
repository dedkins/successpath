class GoalsController < ApplicationController
  ...
  def index
    if member_signed_in?
      @goals_nodate = Goal.where("member_id = ? and goaldate is ?", current_member, nil)
      ...
      @goals_past = Goal.where("member_id = ? and goaldate < ?", current_member, Date.today)
    else
      @goals_nodate = Goal.where("goaldate is ?", nil).limit(3)
      ...
      @goals_past = Goal.where("goaldate < ?", Date.today).limit(2)
    end
